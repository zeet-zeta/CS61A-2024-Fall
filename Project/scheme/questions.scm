  ...
  (cond ((atom? expr)
        ...
        ((let? expr)
         (let ((values (cadr expr))
               (body   (cddr expr)))
           ; BEGIN OPTIONAL PROBLEM 2
           (let ((names-values (zip values)))
            (cons
              (cons 'lambda
                (cons (car names-values)
                  (map let-to-lambda body)))
              (map let-to-lambda (cadr names-values))))
           ; END OPTIONAL PROBLEM 2
           ))
        (else
         ; BEGIN OPTIONAL PROBLEM 2
...
(define (zip pairs)
  (if (null? pairs)
    ...
    (let ((first-pair (car pairs))
          (rest-zipped (zip (cdr pairs))))
      (list 
        (cons (car first-pair) (car rest-zipped))
        (cons (cadr first-pair) (cadr rest-zipped))))))
